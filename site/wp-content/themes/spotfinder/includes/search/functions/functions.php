<?php
	...
	function apply_field_overlay_markup_dropdown($field_id, $value) {
		...
		if($field_markup != '') {
			...
			foreach($fields as $single_field) { if(in_array($single_field->id, $value)) { $string .= $single_field->label.', '; } }
	...
	function apply_field_overlay_markup_dependent($field_id, $value) {
		...
		if($field_markup != '') {
			...
			//// GOES THROUGH EACH FIELD AND CHECKS IF IT"S SELECTED THEN ADDS TO THE STRING
			foreach($fields as $section) { foreach($section as $single_field) { if(in_array($single_field->id, $value)) { $string .= $single_field->label.', '; } } }
