		...
		if($return['error'] == false) {
			...
			if(!is_object($review_id)) {
				...
				if(is_array($terms)) { foreach($terms as $single_field) {
					...
					update_post_meta($review_id, 'sf_rating_field_'.$single_field->term_id, $fields['sf_rating_field_'.$single_field->term_id]);
					
				} }
				
				$current_rating = get_post_meta($fields['post_parent'], 'rating', true);
				if($current_rating == '') { $post = 0; }
				$rating_count = get_post_meta($fields['post_parent'], 'rating_count', true);
				if($rating_count == '') { $post = 0; }
