            	...
            	<div class="<?php echo $the_class; ?><?php if($i%$columns_no == 0) { echo ' last'; } ?> columns" id="search-form-column-<?php echo $i; ?>">
					...
					?>
                    	...
                    	<?php if($the_type == 'text') : //// IF IT'S A TEXT FIELD ?>
                            ...
                            <span class="type-text-wrapper">
							   	...
							   	<input type="text"
									...
									onfocus="if(jQuery(this).val() == '<?php echo get_post_meta($this_field->ID, 'text_default', true); ?>') { jQuery(this).val('') }"
									onblur="if(jQuery(this).val() == '') { jQuery(this).val('<?php echo get_post_meta($this_field->ID, 'text_default', true); ?>') }"
								...
								<?php if(get_post_meta($this_field->ID, 'text_type', true) != 'tags') : ?>
									<input type="hidden" name="_sf_field_<?php echo $this_field->ID ?>_lat" value="" class="_sf_field_places_lat" id="_sf_field_<?php echo $this_field->ID ?>_lat" />
									<input type="hidden" name="_sf_field_<?php echo $this_field->ID ?>_lng" value="" class="_sf_field_places_lng" id="_sf_field_<?php echo $this_field->ID ?>_lng" />
								<?php endif; ?>
                            ...
                            <?php
								...
								<script type="text/javascript">
									...
									jQuery(document).ready(function() {
										...
										<?php if(get_post_meta($this_field->ID, 'text_type', true) == 'tags') : ?>jQuery('#_sf_field_<?php echo $field->id; ?>').btoaLoadTagSuggestions('<?php echo get_post_meta($this_field->ID, 'text_default', true); ?>');
										<?php else : ?>jQuery('#_sf_field_<?php echo $field->id; ?>').btoaLoadGooglePlacesSuggestions('<?php echo get_post_meta($this_field->ID, 'text_default', true); ?>', '<?php echo get_post_meta($this_field->ID, 'google_places_country', true); ?>');<?php endif; ?>
									});
                            ...
                            <select name="<?php echo $this_field->post_name; ?>"<?php if(in_array($field->id, $hidden_array)) : ?> style="display: none;"<?php endif; ?> class="parent-<?php echo $field->id; ?> sf_if_<?php echo $field->id ?> type-dropdown<?php if(get_post_meta($field->id, 'dropdown_change_location', true) == 'on') { echo ' change-location'; } //// IF IT CHANGES THE MAP ?>" onchange="jQuery('#has_changed_<?php echo $this_field->post_name; ?>').val('true'); <?php if(get_post_meta($field->id, 'dropdown_change_location', true)) : ?>jQuery(this)._sf_check_for_radius(<?php echo $field->id; ?>);<?php endif ?> jQuery('#search-spots').submit(); jQuery('#has_changed_<?php echo $this_field->post_name; ?>').val('false');">
                            	...
                            	<?php 
									...
									?>
										...
										<option value="<?php echo $opt->id ?>"<?php if($opt->id == $sel_val) { echo ' selected="selected"'; } ?>><?php echo $opt->label ?></option>
                        ...
                        <?php elseif($field->type == 'min_val') : //// IF IT'S A MIN VAL ?>
                            ...
                            <select name="<?php echo $this_field->post_name; ?>" class="type-min_val sf_if_<?php echo $field->id ?>" onchange="jQuery('#search-spots').submit();"<?php if(in_array($field->id, $hidden_array)) : ?> style="display: none;"<?php endif; ?>>
								...
								?>
                                	...
                                	<option value="<?php echo $opt->value ?>"<?php if($opt->value == $sel_val) { echo ' selected="selected"'; } ?>><?php echo $opt->label ?></option>
                        ...
                        <?php elseif($field->type == 'range') : //// IF IT'S A RANGE ?>
                        	...
                        	<?php if($field->label != '') : ?><label class="sf_if_<?php echo $field->id ?>"<?php if(in_array($field->id, $hidden_array)) : ?> style="display: none;"<?php endif; ?>><?php echo $field->label; ?>
                        ...
                        <?php if($field->label != '') : ?><label class="check-label sf_if_<?php echo $field->id ?>"<?php if(in_array($field->id, $hidden_array)) : ?> style="display: none;"<?php endif; ?>><?php echo $field->label; ?></label><?php endif; //// IF WE HAVE A LABEL ?>
